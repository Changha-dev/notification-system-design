...
5. 미처리 이벤트 **복구 스케줄러**
6. 예약 발송을 위한 **동적 스케줄러**
7. 다중 인스턴스 환경에서 동시 스케줄링 방지를 위한 **ShedLock**
...
./gradlew bootRun
```
- 현재 로컬 기본 설정은 `spring.jpa.hibernate.ddl-auto=create` 이므로, 애플리케이션 재시작 시 스키마가 다시 생성됩니다.
- 따라서 `local` 프로필에서 사용하는 샘플 데이터도 재시작 시 다시 초기화될 수 있습니다.
...
- `status`: 예약 상태 (`PENDING`, `DISPATCHED`)
- 예약 요청 시점에는 이 테이블에만 저장되고, 예약 시각이 되면 실제 `notification`과 `notification_outbox`가 생성됩니다.
#### 회원 통계 테이블
...
```
#### 설명:  
**멱등 키**를 설계하여 중복 요청을 방지하도록 해야겠다고 생각이 들었습니다.  
**가정**: 참조 데이터(referenceId)는 유일한 식별키라고 가정하였습니다. 
- 예외케이스: lectureId가 referenceId로 온다면 제대로된 멱등 키 역할을 하지 못할 수 있기 때문입니다.
...
### 0. 아키텍처 설계
메시지 큐를 이용한 설계 방식도 고려하였지만 **트랜잭션 아웃박스 패턴**과 **커스텀 executor(platform/virtual 전환 가능)** 기반의 비동기 알림 시스템 설계를 채택하였습니다.  
...
- 위에서 파라미터로 고유한 referenceId가 온다고 가정하였기에 서버단에서 멱등키를 설계해도 괜찮다고 판단하였습니다.   
```
...
설계 결정)
1. 멱등 키
- recipientId + notificationType + referenceId + channel로 멱등 키 설계
- recipientId는 수신자 구분, notificationType은 알림의 비즈니스 의미 구분, referenceId는 동일 이벤트 식별, channel은 채널별 발송 구분 용도입니다.
2. 유니크 제약
...
#### 가상 스레드 전환 가능 구조
현재 기본 설정은 `platform` executor 이지만, 설정 변경만으로 `virtual` executor 로 쉽게 교체할 수 있도록 구성하였습니다.
개수가 많은 I/O 작업일 경우 플랫폼 스레드만 활용하는 것보다 가상 스레드가 더 유리할 수 있다고 판단하였습니다.  
플랫폼 스레드는 스레드 개수에 상대적으로 제약이 있지만 가상 스레드는 생성 비용이 매우 가볍기 때문입니다.   
이를 쉽게 교체할 수 있도록 스프링 IoC/DI 개념을 적용하여 설계하였습니다.

### 4. 비즈니스 트랜잭션과 알림 처리 분리
비즈니스 처리와 알림 처리는 서로 영향을 주면 안된다는 요구사항이 있었습니다.  
이를 해결하고자 비즈니스 완료 후 알림 처리 로직이 실행되도록 `@TransactionalEventListener(AFTER_COMMIT)`를 활용하였습니다.
...
구현 방식은 ShedLock의 `LockProvider`를 이용하여 `scheduleId` 단위 DB 분산락을 직접 획득하는 방식으로 처리하였습니다.
